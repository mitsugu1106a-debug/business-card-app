# VERSION: 2026-05-17-FULL-RESTORED
import base64
import secrets
...
# Delayed/lazy imports for heavy dependencies to prevent Render startup crash (Status 1)
# Models and Database are imported safely
import models, database

# Supabase Client Setup (Safe try-except)
...
if SUPABASE_URL and SUPABASE_KEY:
    try:
        ...
        print(f"Supabase init delayed or failed: {e}")

# Watcher Import (Safe try-except)
try:
    import watcher
    HAS_WATCHER = True
except Exception as e:
    print(f"Watcher import warning: {e}")
    HAS_WATCHER = False
...
async def lifespan(app: FastAPI):
    # Enable folder watcher ONLY on local environment (disabled on Render)
    if HAS_WATCHER and not os.getenv("RENDER"):
        ...
        print("Folder watcher is disabled (running on Render or watcher import failed).")
    yield
...
def save_upload_file(upload_file: UploadFile) -> str:
    ...
    # Lazy-loaded PyMuPDF to convert PDF first page to PNG
    if ext == '.pdf':
        try:
            import fitz
    ...
    # Supabase cloud backup if credentials exist
    if supabase_client:
        try:
            with open(final_upload_path, "rb") as f:
                supabase_client.storage.from_("cards").upload(final_filename, f.read())
...
async def upload_async(images: List[UploadFile] = File(...)):
    """Mobile asynchronous bulk upload drop endpoint."""
    ...
    for upload_file in images:
        ...
        try:
            ...
            print(f"[Async Upload] Image dropped: {filename}")
        except Exception as e:
            print(f"Error saving async upload file {filename}: {e}")
...
async def analyze_business_card(image: UploadFile = File(...)):
    """Analyze card image using Google Gemini API."""
    ...
    try:
        ...
        prompt = """
        あなたは名刺解析AIです。
        名刺画像から、写っている全ての情報を抽出し、必ず指定したJSONの【配列（リスト）形式】でのみ出力してください。
        ...
        [
          {
            ...
            "email": "メールアドレス",
            "address": "住所",
            "exchange_date": "名刺交換日 (手書きの日付があればYYYY-MM-DD形式、なければ空文字)",
            "memo": "その他URLやメモ"
          }
        ]
        """
        ...
        for model_name in models_to_try:
            try:
                print(f"Trying Gemini model for OCR: {model_name}")
...
def create_card(
    ...
    if not name and not company_name:
        return {"message": "Skip: Empty inputs."}
...
def update_card(
    ...
    if image and image.filename:
        db_card.image_path = save_upload_file(image)
...
def generate_vcard_response(card_ids: List[str], charset: Optional[str], db: Session):
    ...
    for card in unique_cards:
        ...
        if company_val and name_val: display_name = f"{company_val} {name_val}"
        elif name_val: display_name = name_val
        elif company_val: display_name = company_val
        else: display_name = "名称未設定"
        
        vcf_lines.append(f"FN{c_tag}:{display_name}")
        vcf_lines.append(f"N{c_tag}:{display_name};;;;")
        if company_val: vcf_lines.append(f"ORG{c_tag}:{company_val}")
        if card.title: vcf_lines.append(f"TITLE{c_tag}:{card.title.strip()}")
        if card.department: vcf_lines.append(f"ROLE{c_tag}:{card.department.strip()}")
...
def generate_thunderbird_csv_response(card_ids: List[str], db: Session):
    if not card_ids: raise HTTPException(status_code=400, detail="No card IDs provided.")
    cards = db.query(models.DBBusinessCard).filter(models.DBBusinessCard.id.in_(card_ids)).all()
    output = io.StringIO()
    writer = csv.writer(output)
    ...
    for card in cards:
        name_val = card.name.strip() if card.name else ""
        company_val = card.company_name.strip() if card.company_name else ""
        display_name = f"{company_val} {name_val}" if company_val and name_val else (name_val or company_val)
            ...
            for p in s_nums.split():
                ...
                while len(clean_p) > 0:
                    if clean_p.startswith('090') or clean_p.startswith('080') or clean_p.startswith('070'):
                        ...
                        if not cell_phone: cell_phone = clean_p[:cell_len]
                        clean_p = clean_p[cell_len:]
                    elif clean_p.startswith('050'):
                        work_len = min(11, len(clean_p))
                        if not work_phone: work_phone = clean_p[:work_len]
                        ...
                        if not work_phone: work_phone = clean_p
                        break
        row = [
            name_val, "", display_name, "", card.email or "", "", "", work_phone, "", "", "", cell_phone,
            "", "", "", "", "", "", card.address or "", "", "", "", "", "", company_val, card.title or "",
            card.department or "", "", "", "", "", "", "", "", "", "",
            card.memo.replace('\\', '\\\\').replace('\n', ' ').strip() if card.memo else ""
        ]
        writer.writerow(row)
    ...
    return StreamingResponse(
        iter([content_bytes]),
        media_type="text/csv",
        headers={"Content-Disposition": 'attachment; filename="thunderbird_contacts.csv"'}
    )

@app.post("/manual-import/", response_model=dict)
def manual_import(db: Session = Depends(database.get_db)):
    if not HAS_WATCHER:
        return {"message": "Manual import not available (watcher not loaded).", "results": {"processed": 0, "failed": 0, "skipped": 0}}
    try:
        results = watcher.process_all_pending()
        return {"message": "Manual import complete.", "results": results}
    except Exception as e:
        return {"message": f"Manual import failed: {e}", "results": {"processed": 0, "failed": 0, "skipped": 0}}
...
def generate_csv_response(card_ids: List[str], db: Session):
    ...
    for card in cards:
        ...
        writer.writerow([
            card.id, card.name or "", card.company_name or "", card.department or "", card.title or "",
            card.phone_number or "", card.email or "", card.address or "", card.exchange_date or "",
            card.memo or "", card.image_path or "", tags_str, attachments_str
        ])
    output.seek(0)
    content_bytes = output.getvalue().encode('cp932', errors='replace')
    return StreamingResponse(
        iter([content_bytes]),
        media_type="text/csv",
        headers={"Content-Disposition": 'attachment; filename="business_cards_selected.csv"'}
    )

@app.post("/import-csv/")
async def import_csv(file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    if not file.filename.endswith('.csv'):
        raise HTTPException(status_code=400, detail="Invalid file format. CSV only.")
    try:
        content = await file.read()
        try: text_content = content.decode('utf-8-sig')
        except:
            try: text_content = content.decode('cp932')
            except: text_content = content.decode('cp932', errors='replace')
        ...
        for row in csv_reader:
            ...
            if existing_card:
                ...
                if csv_image_path: existing_card.image_path = csv_image_path.replace('\\', '/')
                updated_count += 1
                ...
                target = new_card
            if "tags" in row:
                sync_tags(db, target, row.get("tags"))
        db.commit()
...
def export_backup():
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=business_cards_full_backup_{timestamp}.zip"}
    )
...
def upload_attachment(card_id: str, file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    db_card = db.query(models.DBBusinessCard).filter(models.DBBusinessCard.id == card_id).first()
    if not db_card: raise HTTPException(status_code=404, detail="Card not found")
    file_path = save_upload_file(file)
    attachment = models.Attachment(card_id=card_id, file_name=file.filename, file_path=file_path)
    db.add(attachment)
    db.commit()
...
# --- UI Server Root (Returns index.html safely) ---
@app.get("/")
def read_root():
    paths = [
    ...
    return {"Hello": "World", "Status": "API is running but UI index.html not found."}
